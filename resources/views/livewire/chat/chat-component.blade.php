                ...
                @forelse ($this->filteredConversations as $conversation)
                    <button wire:click="selectConversation({{ $conversation->id }})"
                        ...
                        <div class="flex items-center space-x-3">
                            <!-- Avatar -->
                            <div class="relative">
                                <div class="w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-semibold shadow-md text-sm lg:text-base">
        ...
        <div class="flex-1 flex flex-col bg-gray-50 dark:bg-gray-800 {{ !$selectedConversation ? 'hidden lg:flex' : 'flex' }}">
                ...
                <div class="flex-1 overflow-y-auto p-3 lg:p-6 space-y-4 lg:space-y-6 pb-20 lg:pb-6">
                    @forelse ($messages as $message)
                        @if ($message->sender_id != auth()->id())
                            ...
                            <div class="flex items-start space-x-2 lg:space-x-3">
                                <div class="w-6 h-6 lg:w-8 lg:h-8 bg-gradient-to-r from-gray-400 to-gray-600 rounded-full flex items-center justify-center text-white text-xs lg:text-sm font-medium flex-shrink-0">
                                    {{ substr($message->sender->name, 0, 1) }}
                                </div>
                                <div class="max-w-[75%] sm:max-w-xs lg:max-w-md">
                                    <div class="bg-white dark:bg-gray-700 rounded-2xl rounded-tl-md px-3 lg:px-4 py-2 lg:py-3 shadow-sm border border-gray-200 dark:border-gray-600">
                                        <p class="text-sm lg:text-base text-gray-900 dark:text-white">{{ $message->content }}</p>
                                    </div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-2 lg:ml-3">{{ $message->created_at->format('H:i') }}</p>
                                </div>
                            </div>
                        @else
                            <!-- Sent Message -->
                            <div class="flex items-start justify-end space-x-2 lg:space-x-3">
                                <div class="max-w-[75%] sm:max-w-xs lg:max-w-md text-right">
                                    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl rounded-tr-md px-3 lg:px-4 py-2 lg:py-3 shadow-sm">
                                        <p class="text-sm lg:text-base text-white">{{ $message->content }}</p>
                                    </div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 mr-2 lg:mr-3">{{ $message->created_at->format('H:i') }}</p>
                                </div>
                                <div class="w-6 h-6 lg:w-8 lg:h-8 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white text-xs lg:text-sm font-medium flex-shrink-0">
                                    Tú
                                </div>
                            </div>
                        @endif
                    @empty
                        <div class="flex flex-col items-center justify-center h-full py-8 lg:py-12">
                            <div class="w-16 h-16 lg:w-24 lg:h-24 bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-full flex items-center justify-center mb-4 lg:mb-6">
                ...
                <div class="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 p-3 lg:p-4 shadow-lg lg:shadow-none">
                    <form wire:submit.prevent="sendMessage" class="flex items-center space-x-2 lg:space-x-4">
                        ...
                        <button type="submit"
                            ...
                            <svg class="w-4 h-4 lg:w-5 lg:h-5 transform group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-6 lg:p-12 text-center">
                    <div class="w-24 h-24 lg:w-32 lg:h-32 bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-full flex items-center justify-center mb-6 lg:mb-8">
                        <svg class="w-12 h-12 lg:w-16 lg:h-16 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                        </svg>
                    </div>
                    <h2 class="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white mb-3 lg:mb-4">Selecciona una conversación</h2>
                    <p class="text-sm lg:text-base text-gray-600 dark:text-gray-400 mb-4 lg:mb-6 max-w-md">Elige una conversación de la lista para comenzar a chatear con tus mentores o estudiantes.</p>
